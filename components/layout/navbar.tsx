    ...
    >
      <div className="container mx-auto px-4">
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              className="md:hidden backdrop-blur-md mt-2 p-4 rounded-xl glass-morphism"
              ...
              <nav className="flex flex-col space-y-4">
                {navItems.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className="text-zinc-400 hover:text-primary transition-colors py-2 px-4 rounded-lg hover:bg-primary/5"
                    onClick={(e) => {
                      e.preventDefault()
                      document.querySelector(item.href)?.scrollIntoView({ behavior: "smooth" })
